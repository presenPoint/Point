...
- `sessions` / `files` / 로그·리포트 테이블 **본인만** RLS 정책 (앱 `upsert`·대시보드용)
- `user_monthly_session_count` 뷰를 **security_invoker** 로 재생성 (Advisor SECURITY DEFINER 경고 해소)
- `get_my_monthly_session_count()` RPC (로그인 사용자 본인 월 횟수)
...
-- 뷰 옵션 확인 (security_invoker = on 이면 OK)
SELECT relname, reloptions
FROM pg_class
WHERE relname = 'user_monthly_session_count';

-- 로그인 JWT로 SQL Editor 테스트 시 (본인만)
SELECT * FROM get_my_monthly_session_count();
```
